...
const SUBTYPES = {
  ...
  'Portland Cement':   ['OPC Type 1', 'PPC', 'Other'],
  'Admixture':         ['Water Reducer', 'Superplasticizer', 'Retarder', 'Accelerator', 'Other'],
  'Fly Ash':           ['Class C', 'Class F', 'Other'],
};
...
function renderGaugeType(md) {
  ...
  Object.keys(SUBTYPES).forEach((mat, i) => {
    ...
    card.innerHTML = `
      ...
      <div class="gauge-wrap"><canvas id="gc-${i}"></canvas></div>
    ...
    setTimeout(() => {
      ...
      gaugeCharts[i] = new Chart(ctx, {
        ...
        options: { responsive:true, maintainAspectRatio:false, cutout:'68%', plugins:{ legend:{display:false}, tooltip:{enabled:false} }, animation:{duration:600} }
      });
    }, 80);
  });
}
...
function openAddForm() {
  ...
  ['m-dr','m-plate','m-supplier','m-type','m-source','m-param','m-result','m-tester','m-remarks'].forEach(id => { document.getElementById(id).value=''; });

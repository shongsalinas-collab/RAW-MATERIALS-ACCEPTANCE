...
function openAddForm() {
  ...
  ['m-dr','m-plate','m-supplier','m-result','m-tester','m-remarks'].forEach(id => { document.getElementById(id).value=''; });
...
function openEditForm(idx) {
  ...
  updateSubtypes(d.subtype||d.type, d.param);
...
function updateSubtypes(selectedSub='', selectedParam='') {
  const mat = document.getElementById('m-material').value;

  // Type dropdown (uses m-subtype element)
  ...
  if (paramEl && paramEl.tagName === 'SELECT') {
...
function saveDelivery() {
  ...
  const entry = {
    ...
    supplier, material,
    type:document.getElementById('m-subtype').value,
